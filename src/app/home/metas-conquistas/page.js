...
export default function MetasConquistasPage() {
  ...
  const updateProgress = async (goal, delta) => {
    const nextCurrent = Math.max(0, Math.min(goal.target_value, goal.current_value + delta));
    ...
    >
      ...
      <section className="max-w-5xl mx-auto space-y-2">
        {goals.map((goal) => {
          ...
          return (
            <article key={goal.id} className="p-3 space-y-2 border rounded-md border-graySm">
              <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  {isEditing ? (
                    <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                      ...
                      <input
                        ...
                        onChange={(e) => setGoalDraft((c) => ({ ...c, category: e.target.value }))}
                      />
                    </div>
                  ) : (
                    <>
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  <button className="px-2 py-1 border rounded-md border-graySm" onClick={() => updateProgress(goal, 1)}>+1</button>
                  <button className="px-2 py-1 border rounded-md border-graySm" onClick={() => updateProgress(goal, -1)}>-1</button>
                  <button className="px-2 py-1 text-white rounded-md bg-primary" onClick={() => completeGoal(goal.id)}>Concluir</button>
                  {isEditing ? (
                    <>
                      <button className="px-2 py-1 text-white rounded-md bg-primary" onClick={() => saveInline(goal)}>Salvar</button>
                      <button className="px-2 py-1 border rounded-md border-graySm" onClick={() => setEditingGoalId(null)}>Cancelar</button>
                    </>
                  ) : (
                    <button className="px-2 py-1 border rounded-md border-graySm" onClick={() => startEditing(goal)}>Editar</button>
                  )}
                  <button className="px-2 py-1 text-white bg-red-500 rounded-md" onClick={() => setGoalToDelete(goal)}>Excluir</button>
                </div>
              </div>
              ...
              <p className="text-xs text-grayMd">
                {goal.current_value}/{goal.target_value} • recompensa: +{goal.reward_coins} moedas e +{goal.reward_xp} XP
              </p>
            </article>
          );
        })}
      ...
      >
        <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
          ...
          <input
            ...
            onChange={(e) => setForm((c) => ({ ...c, target_value: Number(e.target.value) || 1 }))}
          />
          <input
            type="number"
            className="p-2 border rounded-md border-graySm"
            value={form.current_value}
            onChange={(e) => setForm((c) => ({ ...c, current_value: Number(e.target.value) || 0 }))}
          />
          <input
            type="number"
            className="p-2 border rounded-md border-graySm"
            value={form.reward_coins}
            onChange={(e) => setForm((c) => ({ ...c, reward_coins: Number(e.target.value) || 0 }))}
          />
          <input
            type="number"
            className="p-2 border rounded-md border-graySm"
            value={form.reward_xp}
            onChange={(e) => setForm((c) => ({ ...c, reward_xp: Number(e.target.value) || 0 }))}
          />
          <button onClick={createGoal} className="p-2 text-white rounded-md md:col-span-2 bg-primary">
            Criar meta
          </button>
        </div>
      </FormModal>
