  ...
  return (
    ...
    >
      ...
      <section className="max-w-5xl p-3 mx-auto border rounded-md border-graySm bg-white">
        <p className="text-sm">
          Moedas disponíveis para aporte: <strong>{profile?.coins ?? 0}</strong>
        </p>
      </section>
            ...
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="text-xs text-grayMd">
                  Aportado: {item.invested_coins} • Atual: {item.current_value}
                </p>
                <p className="text-xs text-grayMd">
                  Rendimento mensal: {item.monthly_yield_percent}%
                </p>
              </div>
              <div className="flex flex-wrap gap-2 text-xs">
                <button
                  className="px-2 py-1 text-white rounded-md bg-primary"
                  onClick={() => contribute(item.id, 10)}
                >
                  Aportar 10
                </button>
                <button
                  className="px-2 py-1 border rounded-md border-graySm"
                  onClick={() => applyYield(item.id)}
                >
                  Aplicar rendimento
                </button>
                <button
                  className="px-2 py-1 text-white bg-red-500 rounded-md"
                  onClick={() => setInvestmentToDelete(item)}
                >
                  Excluir
                </button>
              </div>
