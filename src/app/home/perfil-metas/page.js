...
export default function PerfilVisaoMetasPage() {
  ...
  const goalsProgress = useMemo(() => {
    ...
    const total = goals.reduce(
      (acc, goal) => acc + (goal.target_value || 0),
      0,
    );
    const current = goals.reduce(
      (acc, goal) => acc + (goal.current_value || 0),
      0,
    );
    if (!total) return 0;
    ...
    >
      <section className="grid max-w-5xl grid-cols-1 gap-3 mx-auto md:grid-cols-2">
        <article className="p-3 border rounded-md border-graySm bg-white">
          ...
          <p className="text-sm">
            Vida: {profile?.hp_current ?? 0}/{profile?.hp_total ?? 0}
          </p>
          <p className="text-sm">
            XP: {profile?.xp_current ?? 0}/{profile?.xp_target ?? 0}
          </p>
        </article>
      ...
      <section className="grid max-w-5xl grid-cols-1 gap-3 mx-auto md:grid-cols-3">
        <article className="p-3 border rounded-md border-graySm bg-white">
          <p className="text-sm">Bons hábitos</p>
          <p className="text-xl">{metrics?.goodHabitsCompleted ?? 0}</p>
        </article>
        <article className="p-3 border rounded-md border-graySm bg-white">
          <p className="text-sm">Maus hábitos</p>
          <p className="text-xl">{metrics?.badHabitsLogged ?? 0}</p>
        </article>
        <article className="p-3 border rounded-md border-graySm bg-white">
          <p className="text-sm">Moedas ganhas</p>
          <p className="text-xl">{metrics?.coinsEarned ?? 0}</p>
        </article>
      </section>
      ...
      <section className="max-w-5xl mx-auto space-y-2">
        {goals.map((goal) => {
          ...
          return (
            <article
              key={goal.id}
              className="p-3 border rounded-md border-graySm bg-white"
            >
