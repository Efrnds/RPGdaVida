...
export default function ThemeSync() {
  useEffect(() => {
    ...
    async function fetchSettings() {
      const [themeResponse, reducedMotionResponse, compactUiResponse, startPageResponse] = await Promise.all([
      ...
      if (reducedMotionResponse.ok) {
        ...
        document.documentElement.dataset.reducedMotion = reducedMotionData?.value ? "1" : "0";
      }

      if (compactUiResponse.ok) {
        const compactUiData = await compactUiResponse.json();
        document.documentElement.dataset.compactUi = compactUiData?.value ? "1" : "0";
      }
      ...
      if (startPageResponse.ok && window.location.pathname === "/home") {
        ...
        if (startPage !== "/home" && startPage.startsWith("/home/")) {
          const redirected = window.sessionStorage.getItem("rpg-start-page-redirected");
          if (!redirected) {
